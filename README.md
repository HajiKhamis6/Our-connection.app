    ...
    <div class="menu-item" onclick="showAdsPreview()">
        ...
        <div class="product-listing-page" id="productListingPage">
            ...
            <div class="product-form-container" id="productFormContainer">
                ...
                <!-- Add a close button -->
        <button class="close-form-btn" onclick="hideProductForm()">
...
async function buyProduct() {
    if (currentProductDetails) {
        ...
        showPaymentOptionsModal(currentProductDetails);
    }
}
...
function showPaymentOptionsModal(product) {
    ...
    const modalContent = `
        <div class="modal" id="paymentOptionsModal">
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header">
                    <h3>Complete Purchase</h3>
                    <span onclick="closePaymentOptionsModal()" style="cursor: pointer;">×</span>
                </div>
                <div class="modal-body">
                    <div class="product-summary" style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                        <h4>${product.name}</h4>
                        <p style="color: #65676B; margin: 5px 0;">Price: $${product.price.toFixed(2)}</p>
                        <p style="color: #65676B; margin: 5px 0;">Seller: ${product.companyName}</p>
                    </div>
                    ...
                    <div class="payment-methods" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px;">
                        <div class="payment-option" onclick="selectPaymentMethod('paypal')" style="padding: 15px; border: 2px solid #eee; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 10px;">
                            <i class="fab fa-paypal" style="font-size: 24px; color: #003087;"></i>
                            <div>
                                <div style="font-weight: bold;">PayPal</div>
                                <div style="font-size: 12px; color: #65676B;">Pay with PayPal account or card</div>
                            </div>
                        </div>
                        
                        <div class="payment-option" onclick="selectPaymentMethod('bitcoin')" style="padding: 15px; border: 2px solid #eee; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 10px;">
                            <i class="fab fa-bitcoin" style="font-size: 24px; color: #F7931A;"></i>
                            <div>
                                <div style="font-weight: bold;">Bitcoin</div>
                                <div style="font-size: 12px; color: #65676B;">Pay with cryptocurrency</div>
                            </div>
                        </div>
                        
                        <div class="payment-option" onclick="selectPaymentMethod('mobile')" style="padding: 15px; border: 2px solid #eee; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 10px;">
                            <i class="fas fa-mobile-alt" style="font-size: 24px; color: #4267B2;"></i>
                            <div>
                                <div style="font-weight: bold;">Mobile Money</div>
                                <div style="font-size: 12px; color: #65676B;">M-Pesa, Tigo Pesa, Airtel Money</div>
                            </div>
                        </div>
                    </div>
                    
                    <!-- PayPal Button Container (hidden by default) -->
                    <div id="paypal-button-container" style="display: none; margin-bottom: 15px;"></div>
                    
                    <!-- Bitcoin Payment Section (hidden by default) -->
                    <div id="bitcoin-payment-section" style="display: none;">
                        <div style="background: #f0f7ff; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                            <h5 style="margin-top: 0;">Bitcoin Payment Instructions</h5>
                            <ol style="margin: 10px 0; padding-left: 20px;">
                                <li>Click the "Pay with Bitcoin" button</li>
                                <li>You'll be redirected to a secure payment page</li>
                                <li>Send the exact amount to the provided address</li>
                                <li>Wait for confirmation (usually takes 10-30 minutes)</li>
                            </ol>
                        </div>
                        <button class="btn btn-primary" onclick="payWithBitcoin()" style="width: 100%;">
                            <i class="fab fa-bitcoin"></i> Pay with Bitcoin
                        </button>
                    </div>
                    
                    <!-- Mobile Money Payment Section (hidden by default) -->
                    <div id="mobile-payment-section" style="display: none;">
                        <div class="payment-instructions" style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                            <h5 style="margin-top: 0;">Mobile Money Payment Instructions:</h5>
                            <ol>
                                <li>Go to your mobile money app (M-Pesa, Tigo Pesa, Airtel Money)</li>
                                <li>Send $${product.price.toFixed(2)} to:</li>
                            </ol>
                        </div>
                        
                        <div class="payment-account" style="background: #e8f4fd; padding: 15px; border-radius: 8px; text-align: center; margin-bottom: 15px;">
                            <div>Mobile Money Account:</div>
                            <div style="font-size: 18px; font-weight: bold; color: #4267B2; margin: 10px 0;">+255776290901</div>
                            <div style="font-size: 14px; color: #65676B;">Reference: PRODUCT-${product.id}</div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" onclick="closePaymentOptionsModal()">Cancel</button>
                </div>
            </div>
        </div>
    `;
...
function selectPaymentMethod(method) {
    ...
    event.currentTarget.style.borderColor = '#4267B2';
    
    // Hide all payment sections
    document.getElementById('paypal-button-container').style.display = 'none';
    document.getElementById('bitcoin-payment-section').style.display = 'none';
    document.getElementById('mobile-payment-section').style.display = 'none';
    ...
    if (method === 'paypal') {
        ...
        document.getElementById('bitcoin-payment-section').style.display = 'block';
    ...
    paypal.Buttons({
        ...
        onApprove: async function(data, actions) {
            return actions.order.capture().then(async function(details) {
                ...
                showToast(`Successfully purchased ${product.name}!`);
            });
        },
        onError: function(err) {
            console.error('PayPal error:', err);
            alert('An error occurred with PayPal payment. Please try again or use another payment method.');
        },
...
async function payWithBitcoin() {
    ...
    // Show loading
    showToast("Processing Bitcoin payment...");
    
    try {
        // In a real app, this would call your backend API
        // For demo purposes, we'll simulate the process
        
        // Simulate API call to create Bitcoin payment
        const paymentResult = await simulateBitcoinPayment(product);
        
        if (paymentResult.success) {
            // Show Bitcoin payment details
            showBitcoinPaymentDetails(paymentResult);
        } else {
            alert("Failed to create Bitcoin payment. Please try again.");
        }
    } catch (error) {
        console.error('Bitcoin payment error:', error);
        alert("An error occurred with Bitcoin payment. Please try another payment method.");
    }
}

// Simulate Bitcoin payment creation
async function simulateBitcoinPayment(product) {
    return new Promise(resolve => {
        setTimeout(() => {
            resolve({
                success: true,
                invoice_url: "https://bitpay.com/invoice?id=demo123",
                bitcoin_address: "1BitcoinAddressForDemoOnly123456",
                amount: product.price,
                currency: "USD"
            });
        }, 1000);
...
// Show Bitcoin payment details
function showBitcoinPaymentDetails(paymentInfo) {
    const modalContent = `
        <div class="modal" id="bitcoinDetailsModal">
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header">
                    <h3>Bitcoin Payment Details</h3>
                    <span onclick="closeBitcoinDetailsModal()" style="cursor: pointer;">×</span>
                </div>
                <div class="modal-body">
                    ...
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                        <div style="margin-bottom: 15px;">
                            <div style="font-size: 12px; color: #65676B; margin-bottom: 5px;">Amount (USD)</div>
                            <div style="font-size: 24px; font-weight: bold; color: #F7931A;">$${paymentInfo.amount.toFixed(2)}</div>
                        </div>
                        
                        <div style="margin-bottom: 15px;">
                            <div style="font-size: 12px; color: #65676B; margin-bottom: 5px;">Bitcoin Address</div>
                            <div style="background: white; padding: 12px; border-radius: 4px; border: 1px solid #ddd; font-family: monospace; word-break: break-all; font-size: 14px;">
                                ${paymentInfo.bitcoin_address}
                            </div>
                            <button onclick="copyBitcoinAddress()" style="margin-top: 10px; background: #f0f2f5; border: none; padding: 8px 15px; border-radius: 4px; cursor: pointer; font-size: 12px;">
                                <i class="fas fa-copy"></i> Copy Address
                            </button>
                        </div>
                        
                        <div style="margin-bottom: 15px;">
                            <div style="font-size: 12px; color: #65676B; margin-bottom: 5px;">QR Code</div>
                            <div style="background: white; padding: 15px; text-align: center; border-radius: 4px; border: 1px solid #ddd;">
                                ...
                                <div style="font-size: 11px; color: #65676B; margin-top: 10px;">(QR code would appear here)</div>
                            </div>
                        </div>
                    </div>
                    
                    <div style="background: #fff0f0; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #e74c3c;">
                        <h5 style="margin-top: 0; color: #e74c3c;">Important Instructions</h5>
                        <ol style="margin: 10px 0; padding-left: 20px; font-size: 13px;">
                            <li>Send the <strong>exact amount</strong> in Bitcoin</li>
                            <li>Use the correct Bitcoin address above</li>
                            <li>Transaction may take 10-30 minutes to confirm</li>
                            <li>Don't send from an exchange (send from your wallet)</li>
                            <li>Keep your transaction ID for reference</li>
                        </ol>
                    </div>
                    
                    <div style="display: flex; gap: 10px;">
                        <button class="btn btn-secondary" onclick="closeBitcoinDetailsModal()" style="flex: 1;">Close</button>
                        <a href="${paymentInfo.invoice_url}" target="_blank" class="btn btn-primary" style="flex: 1;">
    ...
    const existingModal = document.getElementById('bitcoinDetailsModal');
    if (existingModal) {
    ...
    // Show modal
    const modal = document.getElementById('bitcoinDetailsModal');
    if (modal) {
        modal.style.display = 'flex';
...
function copyBitcoinAddress() {
    const bitcoinAddress = document.querySelector('#bitcoinDetailsModal .modal-body div:nth-child(2) div:nth-child(2) div')?.textContent;
    if (bitcoinAddress) {
...
// Close Bitcoin details modal
function closeBitcoinDetailsModal() {
    const modal = document.getElementById('bitcoinDetailsModal');
    if (modal) {
        modal.style.display = 'none';
        setTimeout(() => {
...
// Book Menu Functions - STUDENT ACCOUNT VERSION
...
function closeAllOtherDropdowns(currentDropdown) {
    // Close only the current dropdown, no need to close others in student account
}

// Handle user type selection - STUDENT SPECIFIC
function selectUserType(type) {
    console.log('Student type selected:', type);
    ...
    // Handle different user types - STUDENT VERSION
    if (type === 'new') {
        console.log('Showing student registration');
...
function showToast(message, type = 'info') {
    ...
    const existingToast = document.getElementById('studentToast');
    if (existingToast) {
...
function showStudentRegistration() {
    ...
    // Remove existing modal if any
    const existingModal = document.getElementById('studentRegistrationModal');
    if (existingModal) {
        existingModal.remove();
    }
    
    // Create a modal specifically for student registration
    const modalContent = `
        <div class="modal" id="studentRegistrationModal">
            <div class="modal-content" style="max-width: 600px; max-height: 90vh; overflow-y: auto;">
                ...
                <div class="modal-body">
                    <div style="text-align: center; margin-bottom: 20px;">
                        <i class="fas fa-graduation-cap" style="font-size: 48px; color: #4267B2; margin-bottom: 15px;"></i>
                        <h3>Create Student Account</h3>
                        <p style="color: #65676B; font-size: 14px;">Register as a student to join your school's study group</p>
                    </div>
                    
                    <form id="studentRegistrationForm">
                        <!-- School Information (First for students) -->
                        <div class="form-group">
                            <label for="studentSchool">School/College *</label>
                            <select id="studentSchool" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                                <option value="">Select your institution</option>
                                <option value="kidoti_secondary">Kidoti Secondary School</option>
                                <option value="kidoti_high">Kidoti High School</option>
                                <option value="kidoti_college">Kidoti College</option>
                                <option value="other">Other Institution</option>
                            </select>
                            <input type="text" id="otherInstitution" placeholder="Enter institution name" style="margin-top: 10px; display: none; width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;" maxlength="100">
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                            <div class="form-group">
                                ...
                                <input type="text" id="studentRegNumber" placeholder="e.g., S12345/2024" required maxlength="30">
                                <small style="color: #65676B; font-size: 12px;">Official school registration number</small>
                            </div>
                            
                            <div class="form-group">
                                <label for="studentAcademicYear">Academic Year *</label>
                                <select id="studentAcademicYear" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                        ...
                        <div class="form-group">
                            <label for="studentCombination">Combination/Stream *</label>
                            <select id="studentCombination" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                                <option value="">Select your stream</option>
                                <option value="pcb">PCB (Physics, Chemistry, Biology)</option>
                                <option value="pcm">PCM (Physics, Chemistry, Mathematics)</option>
                                <option value="cbage">CBAGE (Commerce, Bookkeeping, Accounting, Geography, Economics)</option>
                                <option value="hkl">HKL (History, Kiswahili, Literature)</option>
                                <option value="hge">HGE (History, Geography, Economics)</option>
                                <option value="arts">Arts</option>
                                <option value="other">Other</option>
                            </select>
                            <input type="text" id="otherCombination" placeholder="Specify your combination" style="margin-top: 10px; display: none; width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;" maxlength="50">
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                            <div class="form-group">
                                ...
                                <input type="text" id="studentFullName" placeholder="Enter your full name" required maxlength="50">
                            </div>
                            
                            <div class="form-group">
                                <label for="studentDateOfBirth">Date of Birth *</label>
                                <input type="date" id="studentDateOfBirth" required max="${new Date().toISOString().split('T')[0]}">
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                            <div class="form-group">
                                ...
                                <input type="tel" id="studentPhone" placeholder="07XXXXXXXX" required maxlength="13" pattern="[0-9\+]{10,13}">
                                <small style="color: #65676B; font-size: 12px;">Tanzanian number (07XXXXXXXX)</small>
                            </div>
                            
                            <div class="form-group">
                                <label for="studentEmail">Email Address</label>
                                <input type="email" id="studentEmail" placeholder="student@example.com" maxlength="100">
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                            <div class="form-group">
                                ...
                                <input type="password" id="studentPassword" placeholder="Minimum 8 characters" required maxlength="50">
                                <div id="studentPasswordStrength" style="height: 5px; margin-top: 5px; background: #eee; border-radius: 2px; overflow: hidden;">
                                    <div id="studentPasswordStrengthBar" style="height: 100%; width: 0%; background: #e74c3c; transition: width 0.3s;"></div>
                                </div>
                            </div>
                            
                            <div class="form-group">
                                <label for="studentConfirmPassword">Confirm Password *</label>
                                <input type="password" id="studentConfirmPassword" placeholder="Re-enter password" required maxlength="50">
                                <div id="studentPasswordMatch" style="margin-top: 5px; font-size: 12px;"></div>
                            </div>
                        </div>
                        
                        <!-- Profile Picture (Optional for students) -->
                        <div class="form-group" style="text-align: center; margin: 15px 0;">
                            <label style="display: block; margin-bottom: 10px; font-weight: 500;">Student ID Photo (Optional)</label>
                            <div style="position: relative; display: inline-block;">
                                <div id="studentPhotoPreview" style="width: 120px; height: 120px; border-radius: 50%; background: #f0f2f5; display: flex; align-items: center; justify-content: center; margin: 0 auto; border: 2px dashed #ddd; cursor: pointer; overflow: hidden;">
                                    <i class="fas fa-user-graduate" style="font-size: 36px; color: #65676B;"></i>
                                </div>
                                <input type="file" id="studentPhoto" accept="image/*" style="display: none;">
                                <button type="button" class="btn btn-secondary" id="uploadStudentPhotoBtn" style="margin-top: 10px; padding: 6px 15px; font-size: 12px;">
                                    <i class="fas fa-camera"></i> Upload Photo
                                </button>
                            </div>
                            <small style="color: #65676B; font-size: 12px; display: block; margin-top: 5px;">Optional: School ID or profile photo</small>
                        </div>
                        
                        <!-- Student Agreement -->
                        <div style="margin: 15px 0;">
                            <label style="display: flex; align-items: flex-start; cursor: pointer;">
                                <input type="checkbox" id="studentAgreement" style="margin-right: 8px; margin-top: 3px;" required>
                                <span style="font-size: 13px; color: #65676B;">
                                    I agree to follow school rules and maintain respectful behavior in the study group. 
                                    I understand this platform is for educational purposes only.
                                </span>
                            </label>
                        </div>
                        
                        <!-- Submit Button -->
                        <button type="submit" class="btn btn-primary" style="width: 100%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                            <i class="fas fa-user-graduate"></i> Register as Student
                        </button>
                    </form>
                    
                    <div style="border-top: 1px solid #eee; padding-top: 15px; margin-top: 20px;">
                        <p style="text-align: center; color: #65676B; margin-bottom: 10px;">Already have a student account?</p>
                        <button type="button" class="btn btn-secondary" onclick="showStudentLogin()" style="width: 100%;">
                    ...
                    <div style="text-align: center; margin-top: 15px; padding-top: 15px; border-top: 1px solid #eee;">
                        <p style="color: #65676B; font-size: 13px; margin-bottom: 10px;">Looking for regular account?</p>
                        <button type="button" class="btn btn-outline" onclick="switchToRegularAccount()" style="width: 100%; padding: 8px; font-size: 13px; border: 1px solid #ddd; background: transparent;">
                            <i class="fas fa-exchange-alt"></i> Switch to Regular Account
                        </button>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    if (modal) {
        ...
        const otherSchoolInput = document.getElementById('otherInstitution');
        const combinationSelect = document.getElementById('studentCombination');
        const otherCombinationInput = document.getElementById('otherCombination');
        const passwordInput = document.getElementById('studentPassword');
        const confirmPasswordInput = document.getElementById('studentConfirmPassword');
        const uploadPhotoBtn = document.getElementById('uploadStudentPhotoBtn');
        ...
        if (combinationSelect && otherCombinationInput) {
            combinationSelect.addEventListener('change', function() {
                console.log('Combination selected:', this.value);
                if (this.value === 'other') {
                    otherCombinationInput.style.display = 'block';
                    otherCombinationInput.required = true;
                } else {
                    otherCombinationInput.style.display = 'none';
                    otherCombinationInput.required = false;
                }
            });
        }
        
        if (passwordInput) {
            passwordInput.addEventListener('input', checkStudentPasswordStrength);
        }
        
        if (confirmPasswordInput) {
            confirmPasswordInput.addEventListener('input', checkStudentPasswordMatch);
        }
        
        if (uploadPhotoBtn && studentPhotoInput) {
        ...
        if (studentPhotoInput) {
            studentPhotoInput.addEventListener('change', previewStudentPhoto);
        }
        
        // Auto-focus on school select
        setTimeout(() => {
            const schoolInput = document.getElementById('studentSchool');
            if (schoolInput) {
                schoolInput.focus();
                console.log('School input focused');
            }
        }, 100);
    }
}
...
function showStudentLogin() {
    console.log('Opening student login modal');
    
    // Close registration modal if open
    const regModal = document.getElementById('studentRegistrationModal');
    if (regModal) {
        regModal.remove();
    }
    ...
    const modalContent = `
        <div class="modal" id="studentLoginModal">
            <div class="modal-content" style="max-width: 500px;">
                ...
                <div class="modal-body">
                    <div style="text-align: center; margin-bottom: 20px;">
                        <i class="fas fa-graduation-cap" style="font-size: 48px; color: #42B983; margin-bottom: 15px;"></i>
                        <h3>Welcome Back, Student!</h3>
                        <p style="color: #65676B;">Login to access your study group</p>
                    </div>
                    
                    <form id="studentLoginForm">
                        <!-- Student-specific login options -->
                        <div class="form-group">
                            <label for="studentLoginType">Login Using:</label>
                            <select id="studentLoginType" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;" onchange="toggleStudentLoginFields()">
                        ...
                        <div class="form-group">
                            <div id="studentLoginFieldContainer">
                                <input type="text" id="studentLoginRegNumber" placeholder="Enter registration number" required class="student-login-field" style="display: block;">
                                <input type="tel" id="studentLoginPhone" placeholder="07XXXXXXXX" class="student-login-field" style="display: none;">
                                <input type="email" id="studentLoginEmail" placeholder="student@example.com" class="student-login-field" style="display: none;">
                            </div>
                        </div>
                        
                        <div class="form-group">
                            <label for="studentLoginPassword">Password</label>
                            <input type="password" id="studentLoginPassword" placeholder="Enter your password" required>
                        </div>
                        
                        <div style="margin: 15px 0;">
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="checkbox" id="studentRememberMe" style="margin-right: 8px;">
                                <span style="font-size: 13px; color: #65676B;">Remember me on this device</span>
                            </label>
                        </div>
                        
                        <div style="text-align: center; margin-top: 10px;">
                            <a href="#" onclick="showStudentPasswordRecovery(); return false;" style="color: #4267B2; text-decoration: none; font-size: 13px;">
                                <i class="fas fa-key"></i> Forgot Password?
                            </a>
                        </div>
                        
                        <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                            <i class="fas fa-sign-in-alt"></i> Student Login
                        </button>
                    </form>
                    
                    <div style="border-top: 1px solid #eee; padding-top: 15px; margin-top: 20px;">
                        <p style="text-align: center; color: #65676B; margin-bottom: 10px;">New student?</p>
                        <button type="button" class="btn btn-secondary" onclick="showStudentRegistration()" style="width: 100%;">
    ...
    if (modal) {
        ...
        toggleStudentLoginFields();
        
        // Auto-focus on first field
        setTimeout(() => {
            const field = document.querySelector('.student-login-field[style*="display: block"]');
            if (field) {
                field.focus();
                console.log('Login field focused');
            }
        }, 100);
    }
}

// Toggle student login fields
function toggleStudentLoginFields() {
    const loginType = document.getElementById('studentLoginType');
    const fields = document.querySelectorAll('.student-login-field');
    
    if (!loginType) return;
    
    console.log('Toggling login fields to:', loginType.value);
    ...
    switch(loginType.value) {
        case 'regNumber':
            document.getElementById('studentLoginRegNumber').style.display = 'block';
            document.getElementById('studentLoginRegNumber').required = true;
            break;
        case 'phone':
            document.getElementById('studentLoginPhone').style.display = 'block';
            document.getElementById('studentLoginPhone').required = true;
            break;
        case 'email':
            document.getElementById('studentLoginEmail').style.display = 'block';
            document.getElementById('studentLoginEmail').required = true;
            break;
    }
}
...
function checkStudentPasswordStrength() {
    ...
    const strengthBar = document.getElementById('studentPasswordStrengthBar');
...
function checkStudentPasswordMatch() {
    const password = document.getElementById('studentPassword');
    const confirmPassword = document.getElementById('studentConfirmPassword');
...
function previewStudentPhoto(event) {
    console.log('Previewing student photo');
...
function registerStudent() {
    ...
    const otherSchool = document.getElementById('otherInstitution');
    const regNumber = document.getElementById('studentRegNumber');
    const academicYear = document.getElementById('studentAcademicYear');
    const combination = document.getElementById('studentCombination');
    const otherCombination = document.getElementById('otherCombination');
    ...
    const password = document.getElementById('studentPassword');
    const confirmPassword = document.getElementById('studentConfirmPassword');
    const agreement = document.getElementById('studentAgreement');
    ...
    let institutionName;
    if (school.value === 'other') {
        if (!otherSchool?.value || otherSchool.value.trim().length < 2) {
            showToast("Please specify your institution name", "error");
            if (otherSchool) otherSchool.focus();
            return;
        }
        institutionName = otherSchool.value.trim();
    } else {
        institutionName = school.options[school.selectedIndex].text;
    }
    
    if (!regNumber?.value || regNumber.value.length < 3) {
        showToast("Please enter your registration number", "error");
        regNumber?.focus();
        return;
    }
    ...
    if (!combination?.value) {
        showToast("Please select your combination/stream", "error");
        combination?.focus();
        return;
    }
    
    let combinationName;
    if (combination.value === 'other') {
        if (!otherCombination?.value || otherCombination.value.trim().length < 2) {
            showToast("Please specify your combination", "error");
            if (otherCombination) otherCombination.focus();
            return;
        }
        combinationName = otherCombination.value.trim();
    } else {
        combinationName = combination.options[combination.selectedIndex].text;
    }
    ...
    // Validate age (must be at least 10 for student)
    ...
    if (age < 10) {
        showToast("You must be at least 10 years old to register as a student", "error");
        dateOfBirth?.focus();
        return;
    }
    
    if (age > 25) {
        showToast("Student registration is for ages 10-25 only", "error");
        dateOfBirth?.focus();
        return;
    }
    ...
    if (!phoneRegex.test(cleanedPhone)) {
        showToast("Please enter a valid Tanzanian phone number (07XXXXXXXX)", "error");
        phone?.focus();
        return;
    }
    ...
    let studentEmail = email?.value ? email.value.trim() : '';
    ...
    if (!agreement?.checked) {
        showToast("Please agree to the student terms", "error");
        return;
    }
    ...
    let students = JSON.parse(localStorage.getItem('oc_students')) || [];
    console.log("Existing students:", students.length);
    
    // Check registration number
    const existingRegStudent = students.find(s => s.regNumber === regNumber.value);
    if (existingRegStudent) {
        showToast("This registration number is already registered", "error");
        regNumber?.focus();
        return;
    }
    
    // Check phone number
    const existingPhoneStudent = students.find(s => s.phone === normalizedPhone);
    ...
    if (studentEmail) {
        const existingEmailStudent = students.find(s => s.email === studentEmail);
    ...
    if (photoInput && photoInput.files && photoInput.files[0]) {
        console.log("Photo uploaded, processing...");
        ...
        reader.onload = function(e) {
            ...
            completeStudentRegistration(photoData);
        };
        reader.onerror = function() {
            console.log("Error loading photo, continuing without photo");
    ...
    function completeStudentRegistration(photo) {
        console.log("Completing student registration...");
        ...
        localStorage.setItem('oc_students', JSON.stringify(students));
        console.log("Student saved to localStorage");
        
        // Create or join institution group
        let groups = JSON.parse(localStorage.getItem('oc_student_groups')) || [];
        let studentGroup = groups.find(g => g.institution === institutionName);
        
        if (!studentGroup) {
            console.log("Creating new student group for:", institutionName);
            // First student from this institution becomes group admin
            const groupId = 'student_group_' + institutionName.toLowerCase().replace(/[^a-z0-9]/g, '_') + '_' + Date.now();
            studentGroup = {
                id: groupId,
                name: institutionName + " Study Group",
                institution: institutionName,
                admin: studentId,
                members: [studentId],
                type: 'student',
            ...
            localStorage.setItem('oc_student_groups', JSON.stringify(groups));
            
            // Initialize group messages
            localStorage.setItem(`student_messages_${groupId}`, JSON.stringify([]));
            console.log("New group created:", studentGroup.name);
        } else {
            console.log("Joining existing group:", studentGroup.name);
            // Add student to existing group
            if (!studentGroup.members.includes(studentId)) {
                studentGroup.members.push(studentId);
                localStorage.setItem('oc_student_groups', JSON.stringify(groups));
        ...
        console.log("Student logged in as current user");
        
        showToast(`✅ Welcome ${fullName.value}! Student account created successfully`, "success");
        closeStudentModal();
        
        // Navigate to student group chat
        setTimeout(() => {
            console.log("Navigating to student group chat...");
            navigateToStudentGroup(studentData, studentGroup);
        }, 1500);
    }
}
...
function loginStudent() {
    ...
    switch(loginType.value) {
        case 'regNumber':
            const regNumber = document.getElementById('studentLoginRegNumber');
            if (!regNumber || !regNumber.value.trim()) {
                showToast("Please enter your registration number", "error");
                if (regNumber) regNumber.focus();
                return;
            }
            identifier = regNumber.value.trim();
            console.log("Login with registration number:", identifier);
            break;
        case 'phone':
            const phone = document.getElementById('studentLoginPhone');
            if (!phone || !phone.value.trim()) {
                showToast("Please enter your phone number", "error");
                if (phone) phone.focus();
                return;
            }
            identifier = phone.value.trim().replace(/\s/g, '').replace(/^\+/, '');
        ...
        case 'email':
            ...
            break;
        default:
            showToast("Invalid login type", "error");
            return;
    }
    ...
    // Find student
    const students = JSON.parse(localStorage.getItem('oc_students')) || [];
    console.log("Total students in system:", students.length);
    let student;
    
    switch(loginType.value) {
        case 'regNumber':
            student = students.find(s => s.regNumber === identifier && atob(s.password) === password.value);
            break;
        case 'phone':
            // Normalize phone for comparison
            const normalizedPhone = identifier.startsWith('255') ? identifier : 
                                   identifier.startsWith('07') ? '255' + identifier.substring(1) : 
                                   identifier.startsWith('7') ? '255' + identifier : identifier;
            student = students.find(s => s.phone === normalizedPhone && atob(s.password) === password.value);
            break;
        case 'email':
            student = students.find(s => s.email === identifier && atob(s.password) === password.value);
            break;
    }
    ...
    const groups = JSON.parse(localStorage.getItem('oc_student_groups')) || [];
    const studentGroup = groups.find(g => g.members.includes(student.id));
    ...
    const rememberMe = document.getElementById('studentRememberMe');
    ...
    setTimeout(() => {
        if (studentGroup) {
            console.log("Navigating to existing group:", studentGroup.name);
            navigateToStudentGroup(student, studentGroup);
        } else {
            console.log("Creating temporary group for student");
            // Create temporary group if none exists
            const tempGroup = {
                id: 'temp_group_' + student.id,
                name: student.institution + " Study Group",
                institution: student.institution,
                admin: student.id,
                members: [student.id],
                type: 'student',
            ...
            navigateToStudentGroup(student, tempGroup);
        }
    }, 1000);
}

// Navigate to Student Group
function navigateToStudentGroup(student, group) {
    console.log("=== Navigating to Student Group ===");
    ...
    if (groupPage) groupPage.style.display = 'none';
    
    // Show student-specific group page
    let studentGroupPage = document.getElementById('studentGroupPage');
    
    if (!studentGroupPage) {
        console.log("Creating student group page");
        // Create student group page if it doesn't exist
        const pageContent = `
            <div id="studentGroupPage" style="display: flex; flex-direction: column; height: 100vh; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: white; z-index: 1000;">
                <div class="group-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; display: flex; align-items: center; gap: 15px;">
                    <img src="${student.photo || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(student.name) + '&background=667eea&color=fff'}" alt="Profile" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover;">
                    <div style="flex: 1;">
                        <h4 style="margin: 0; font-size: 16px;">${group.name}</h4>
                        <small style="opacity: 0.9;">${student.name} • ${student.academicYear.toUpperCase()}</small>
                    </div>
                    <div>
                        <button onclick="logoutStudent()" style="background: rgba(255,255,255,0.2); border: none; color: white; padding: 8px 15px; border-radius: 4px; cursor: pointer; font-size: 12px;">
                            <i class="fas fa-sign-out-alt"></i> Logout
                        </button>
                    </div>
                </div>
                
                <div id="studentChatArea" style="flex: 1; padding: 15px; overflow-y: auto; background: #f8f9fa;"></div>
                
                <div style="padding: 15px; background: white; border-top: 1px solid #eee;">
                    <div style="display: flex; gap: 10px;">
                        <input type="text" id="studentMsg" placeholder="Type your message..." style="flex: 1; padding: 12px; border: 1px solid #ddd; border-radius: 25px; font-size: 14px;">
                        <button onclick="sendStudentMsg()" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; width: 45px; height: 45px; border-radius: 50%; cursor: pointer;">
                            <i class="fas fa-paper-plane"></i>
                        </button>
                    </div>
                    <div style="text-align: center; margin-top: 10px;">
                        <small style="color: #999; font-size: 12px;">This group is for ${student.institution} students only</small>
                    </div>
                </div>
            </div>
        `;
        ...
        document.body.appendChild(pageDiv.firstElementChild);
        studentGroupPage = document.getElementById('studentGroupPage');
    }
    
    if (studentGroupPage) {
        console.log("Student group page found/created, updating...");
        // Update page with current student info
        const headerImg = studentGroupPage.querySelector('.group-header img');
        const headerTitle = studentGroupPage.querySelector('.group-header h4');
        const headerSubtitle = studentGroupPage.querySelector('.group-header small');
        
        if (headerImg && student.photo) {
            headerImg.src = student.photo;
        } else if (headerImg) {
            headerImg.src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(student.name) + '&background=667eea&color=fff';
        }
        
        if (headerTitle) headerTitle.textContent = group.name;
        if (headerSubtitle) headerSubtitle.textContent = `${student.name} • ${student.academicYear.toUpperCase()}`;
        
        // Show student group page
        studentGroupPage.style.display = 'flex';
        
        // Load student messages
        loadStudentMessages(group.id);
        
        // Focus on message input
        setTimeout(() => {
            const msgInput = document.getElementById('studentMsg');
            if (msgInput) {
                msgInput.focus();
    ...
    console.log("Student group navigation complete");
}

// Load Student Messages
function loadStudentMessages(groupId) {
    console.log("Loading messages for group:", groupId);
    const chatArea = document.getElementById('studentChatArea');
    if (!chatArea) {
        console.error("Chat area not found");
        return;
    }
    
    const messages = JSON.parse(localStorage.getItem(`student_messages_${groupId}`)) || [];
    ...
    if (messages.length === 0) {
        chatArea.innerHTML = `
            <div style="text-align: center; padding: 40px 20px; color: #999;">
                ...
                <p>Welcome to your study group! Start the conversation.</p>
                <small style="display: block; margin-top: 10px;">Share notes, ask questions, and collaborate with classmates</small>
            </div>
        `;
        return;
    }
    ...
    messages.forEach(msg => {
        ...
        div.className = `message ${msg.studentId === window.currentStudent?.id ? 'me' : 'other'}`;
        div.style.cssText = `
            margin-bottom: 15px;
            padding: 10px 15px;
            border-radius: 18px;
            max-width: 70%;
            position: relative;
            ${msg.studentId === window.currentStudent?.id ? 
                'background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; margin-left: auto;' : 
                'background: white; color: #333; margin-right: auto; border: 1px solid #ddd;'}
        `;
        
        // Find sender info
        const students = JSON.parse(localStorage.getItem('oc_students')) || [];
        const sender = students.find(s => s.id === msg.studentId);
        const senderName = sender ? sender.name : 'Unknown Student';
        
        const time = new Date(msg.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        
        div.innerHTML = `
            <div class="message-header" style="font-size: 12px; margin-bottom: 5px; opacity: 0.8;">
                <strong>${senderName}${sender && sender.id === window.currentStudent?.id ? ' (You)' : ''}</strong>
                <span class="message-time" style="margin-left: 10px;">${time}</span>
            </div>
            <div class="message-text" style="font-size: 14px;">${msg.text}</div>
            ${msg.studentId === window.currentStudent?.id ? '<div style="font-size: 11px; opacity: 0.7; text-align: right; margin-top: 5px;">✓ Delivered</div>' : ''}
        `;
...
function sendStudentMsg() {
    ...
    const input = document.getElementById('studentMsg');
    ...
    // Find student's group
    const groups = JSON.parse(localStorage.getItem('oc_student_groups')) || [];
    const studentGroup = groups.find(g => g.members.includes(currentStudent.id));
    
    if (!studentGroup) {
        showToast("You are not in any study group", "error");
        return;
    }
    
    console.log("Sending message to group:", studentGroup.name);
    
    const messages = JSON.parse(localStorage.getItem(`student_messages_${studentGroup.id}`)) || [];
    const newMessage = {
    ...
    localStorage.setItem(`student_messages_${studentGroup.id}`, JSON.stringify(messages));
    
    console.log("Message sent:", {
        student: currentStudent.name,
        text: input.value.trim().substring(0, 50) + '...',
        group: studentGroup.name
    });
    
    input.value = '';
    loadStudentMessages(studentGroup.id);
}

// Student Password Recovery
function showStudentPasswordRecovery() {
    console.log("Showing password recovery");
    const modal = document.getElementById('studentLoginModal');
    if (modal) {
        ...
        modalBody.innerHTML = `
            <div style="text-align: center; margin-bottom: 20px;">
                ...
                <p style="color: #65676B;">Enter your registration number to reset password</p>
            </div>
            
            <form id="studentRecoveryForm">
                <div class="form-group">
                    <label for="studentRecoveryRegNumber">Registration Number *</label>
                    <input type="text" id="studentRecoveryRegNumber" placeholder="Enter your registration number" required>
                </div>
                
                <div class="form-group">
                    <label for="studentRecoveryPhone">Phone Number *</label>
                    <input type="tel" id="studentRecoveryPhone" placeholder="07XXXXXXXX" required>
                    <small style="color: #65676B; font-size: 12px;">Must match the number in your registration</small>
                </div>
            ...
            <div style="text-align: center; margin-top: 20px;">
                <button type="button" class="btn btn-secondary" onclick="showStudentLogin()">
        ...
        // Add form handler
        const form = document.getElementById('studentRecoveryForm');
        if (form) {
            form.addEventListener('submit', function(e) {
                e.preventDefault();
                console.log("Password recovery form submitted");
                const regNumber = document.getElementById('studentRecoveryRegNumber').value.trim();
                const phone = document.getElementById('studentRecoveryPhone').value.trim().replace(/\s/g, '').replace(/^\+/, '');
                ...
                if (student) {
                    // Normalize phone for comparison
                    const normalizedPhone = phone.startsWith('255') ? phone : 
                                          phone.startsWith('07') ? '255' + phone.substring(1) : 
                                          phone.startsWith('7') ? '255' + phone : phone;
                    
                    if (student.phone === normalizedPhone) {
                        console.log("Student verified for password reset:", student.name);
                        // Generate temporary password
                        const tempPassword = Math.random().toString(36).slice(-8);
                        student.password = btoa(tempPassword);
                        ...
                        if (studentIndex !== -1) {
                            ...
                            showToast(`Password reset! Your temporary password: ${tempPassword}`, "success");
                            showStudentLogin();
                        }
                    } else {
                        showToast("Phone number doesn't match registration", "error");
                    }
                } else {
...
function logoutStudent() {
    ...
    if (window.currentStudent) {
        ...
        console.log("Student logged out:", window.currentStudent.name);
        window.currentStudent = null;
        localStorage.removeItem('current_student');
        
        // Hide student group page
        const studentGroupPage = document.getElementById('studentGroupPage');
        if (studentGroupPage) {
            studentGroupPage.style.display = 'none';
            console.log("Student group page hidden");
        }
...
function closeStudentModal() {
    ...
    if (regModal) {
        ...
        setTimeout(() => {
            if (regModal.parentNode) {
                regModal.remove();
                console.log("Registration modal removed");
            }
        }, 300);
    }
...
// Switch to Regular Account
function switchToRegularAccount() {
    console.log("Switching to regular account");
    closeStudentModal();
    setTimeout(() => {
        // Check if regular account functions exist
        if (typeof showAccountPopup === 'function') {
            showAccountPopup();
        } else if (typeof selectUserType === 'function') {
            // This might be the regular account version
            selectUserType('new');
        } else {
            console.error("Regular account functions not found");
            showToast("Regular account system not available", "error");
        }
    }, 300);
}

// Initialize Student Account System
function initStudentAccountSystem() {
    console.log("=== Initializing Student Account System ===");
    ...
    if (savedStudent) {
        try {
            ...
            if (student) {
                ...
                if (studentGroup) {
                    console.log("Auto-login detected for student:", student.name);
                    navigateToStudentGroup(student, studentGroup);
                } else {
                    console.log("Student has no group, creating temporary");
                    const tempGroup = {
                        id: 'temp_group_' + student.id,
                        name: student.institution + " Study Group",
                        institution: student.institution,
                ...
                console.log("Saved student not found in database");
                localStorage.removeItem('current_student');
            }
        } catch (e) {
...
if (!document.querySelector('#student-styles')) {
    ...
    `;
    document.head.appendChild(style);
    console.log("Student styles added");
}

// Call initialization when page loads
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM loaded, initializing student system...");
    initStudentAccountSystem();
});

// Also initialize if DOM is already loaded
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initStudentAccountSystem);
} else {
    initStudentAccountSystem();
}
...
window.toggleStudentLoginFields = toggleStudentLoginFields;
window.checkStudentPasswordStrength = checkStudentPasswordStrength;
window.checkStudentPasswordMatch = checkStudentPasswordMatch;
window.previewStudentPhoto = previewStudentPhoto;
window.registerStudent = registerStudent;
window.loginStudent = loginStudent;
window.navigateToStudentGroup = navigateToStudentGroup;
window.loadStudentMessages = loadStudentMessages;
window.sendStudentMsg = sendStudentMsg;
window.showStudentPasswordRecovery = showStudentPasswordRecovery;
window.logoutStudent = logoutStudent;
window.closeStudentModal = closeStudentModal;
window.switchToRegularAccount = switchToRegularAccount;
window.initStudentAccountSystem = initStudentAccountSystem;

console.log("Student account system loaded and ready");
